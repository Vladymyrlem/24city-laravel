<div id="post-{{ $post->id }}" class="d-flex flex-row slide-content carousel-post news type-news status-publish">
    @if(!empty($post->image))
    ...
    <div class="slide-title d-flex flex-column" style="">
        <div class="d-flex flex-row justify-content-between head-slide">
            <a class="post-link" href="{{ route('news.show', $post->id) }}" redirect>
                {{ $post->title }}
            </a>
