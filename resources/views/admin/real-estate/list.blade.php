    ...
    <table class="table table-bordered table-hover table-dark">
        ...
        @foreach($real_estate as $post)
            ...
            <tr>
                ...
                <td><a href="{{ route('admin.real-estate.show', $post->slug) }}">{{ $post->title }}</a></td>
                ...
                <td>
                    @foreach ($post->tags as $tag)
                        <strong><a href="{{ route('admin.real-estate-tag-show', $tag->id) }}">{{ $tag->name }}</a>
                        </strong>
                    @endforeach
                </td>
                <td>
                    {{--                    @dd($post->categories)--}}
                    {{--                    @if($post->categories instanceof \Illuminate\Database\Eloquent\Collection)--}}
                    ...
                    {{--                    @foreach ($post->categories as $category)--}}
                    {{--                        <strong><a--}}
                    {{--                                href="{{ route('admin.real-estate-category-show', $category->id) }}">{{ $category->name }}</a>--}}
                    {{--                        </strong>--}}
                    {{--                    @endforeach--}}
                </td>
                <td>{{$post->created_at}}</td>
                <td class="actions d-flex">
                    <a href="{{ route('admin.real-estate.delete', $post->id) }}" class="btn btn-danger">Delete</a>&nbsp;
                    <a href="{{ route('admin.real-estate.edit', $post->id) }}"
...
<script>
    {{--var real_estateCategories = <?php echo json_encode($real_estateCategories); ?>;--}}
    {{--//console.log(real_estateCategories);--}}
    {{--// console.log(companyCategory);--}}
    {{--const result = {};--}}

    {{--// Loop through the input array--}}
    {{--for (const key in real_estateCategories) {--}}
    {{--    if (real_estateCategories.hasOwnProperty(key)) {--}}
    {{--        const item = real_estateCategories[key];--}}
...
<?php
//    $cats = json_decode($main_real_estate);
